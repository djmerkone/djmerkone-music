        ...
        <div>
          ...
          <div className="block">
            <DynamicShadowText 
              ...
              className={`text-6xl font-black uppercase tracking-tighter ${artist.isMemorial ? 'text-zinc-400 italic' : 'text-white group-hover:text-red-500'}`}
            />
          </div>
          {artist.isMemorial && (
            <p className="mono text-[10px] text-zinc-600 uppercase tracking-widest mt-4 leading-loose max-w-xs italic text-white">Carlos 'Charlie' Velasquez</p>
          )}
        </div>
...
const App = () => {
  ...
  const releases = [
    { title: "Don't Let Me", artist: "Marilyn Torres", type: "EP (8 Tracks)", yt: "https://music.youtube.com/watch?v=Xh0mE3n_38w" },
    ...
    <div className="min-h-screen bg-[#030303] text-zinc-100 font-sans selection:bg-red-500/30 overflow-x-hidden">
      <style>
        {`
          ...
          .hub-bg {
            background-image: linear-gradient(to right, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.6) 100%), url("/loumerk.png");
      ...
      {/* Studio Section (The Hub) - Uses loumerk.png */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
           <div className="lg:col-span-5 p-16 border border-white/10 bg-black/70 backdrop-blur-xl rounded-[3rem] h-full flex flex-col justify-between hover:border-red-500/30 transition-all group">
              ...
              <div>
                ...
                <p className="text-zinc-200 text-sm font-bold leading-relaxed uppercase tracking-widest italic border-l border-white/20 pl-6 text-white">Engineering multi-genre fidelity since 2019.</p>
              </div>
           </div>
            ...
            {services.map((service, i) => (
              <div key={i} className="p-10 border border-white/10 bg-black/85 backdrop-blur-md rounded-[2rem] hover:bg-zinc-900/60 transition-all group flex flex-col justify-between">
                ...
                <div>
                  ...
                  <p className="text-zinc-300 text-xs font-medium tracking-widest leading-relaxed text-white">{service.detail}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="work" className="py-40 px-8 md:pl-40 md:pr-24 border-t border-white/5 bg-[#030303]">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-32">
          <div className="sticky top-32">
             ...
             <p className="mono text-xs text-zinc-500 uppercase tracking-widest italic leading-loose max-w-sm text-white">
               Clinical sonic output across the djmerkone MUSIC network. Select a track to listen on YouTube Music.
             </p>
          </div>
          ...
          <div className="space-y-4 max-h-[120vh] overflow-y-auto pr-4 custom-scrollbar">
             {releases.map((track, i) => (
               <a 
                 ...
                 className="group flex items-center p-10 border border-white/5 bg-zinc-950/50 hover:bg-white/5 transition-all cursor-pointer"
               >
                  <Disc className="text-zinc-800 group-hover:text-red-500 mr-10 animate-spin-slow" size={32} />
                  <div className="flex-grow">
                    ...
                    <div className="flex items-center space-x-4 mt-2">
                      <p className="mono text-base text-zinc-400 uppercase italic lowercase font-bold text-white">{track.artist}</p>
                  ...
                  <div className="flex items-center">
                    <div className="w-12 h-12 border border-white/5 flex items-center justify-center rounded-full group-hover:bg-red-600 group-hover:border-red-600 group-hover:text-white transition-all">
      ...
      <footer className="bg-black py-40 px-8 md:pl-40 md:pr-24 border-t border-white/5 relative overflow-hidden text-white">
          ...
          <div className="grid grid-cols-2 gap-20">
            <div>
              ...
              <ul className="space-y-4 text-xs font-black uppercase tracking-widest text-zinc-400 italic text-white">
                ...
                <li><a href="https://djmerkone-site0.vercel.app" target="_blank" className="hover:text-red-500 flex items-center transition-colors text-white lowercase italic text-white">djmerkone <ExternalLink size={12} className="ml-2" /></a></li>
              </ul>
            </div>
            <div>
        ...
        <div className="relative z-10 pt-40 flex flex-col md:flex-row justify-between items-center text-zinc-700 mono text-[9px] uppercase tracking-widest font-black text-white">
          ...
          <div className="relative w-full max-w-5xl max-h-[90vh] bg-zinc-950 border border-white/10 rounded-[3rem] overflow-hidden flex flex-col shadow-2xl shadow-black animate-in fade-in zoom-in duration-300 text-white">
            <div className="p-8 border-b border-white/5 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div className="mono text-[10px] tracking-[0.4em] uppercase text-zinc-500 text-white">
                  djmerkone MUSIC // {modal.type === 'artist' ? (modal.data.isMemorial ? 'Memorial_Memorandum' : 'Artist_Profile') : 'Information_Hub'}
                </div>
              </div>
              <button onClick={closeModal} className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-white hover:text-black transition-all text-white">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-grow overflow-y-auto p-10 custom-scrollbar text-white">
              {modal.type === 'artist' ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start text-white">
                  ...
                  <div className="space-y-12">
                    <div className="prose prose-invert max-w-none text-white">
                      ...
                      <p className="mono text-[11px] leading-relaxed tracking-widest uppercase text-zinc-400 italic whitespace-pre-line text-white">
                        {modal.data.bio}
                      </p>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-6 text-white">
                        {modal.data.link && (
                          <a href={modal.data.link} target="_blank" className="flex items-center space-x-6 group">
                            <div className="w-10 h-10 border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all text-white">
                        ...
                        {modal.data.socials && (
                          <div className="flex flex-wrap gap-6 pt-4 text-white">
                            {modal.data.socials.fb && (
                              <a href={`https://facebook.com/${modal.data.socials.fb}`} target="_blank" className="text-zinc-500 hover:text-white transition-colors text-white">
                                <Facebook size={20} />
                              </a>
                            )}
                            {modal.data.socials.ig && (
                              <a href={`https://instagram.com/${modal.data.socials.ig}`} target="_blank" className="text-zinc-500 hover:text-white transition-colors text-white">
                                <Instagram size={20} />
                              </a>
                            )}
                            {modal.data.socials.tt && (
                              <a href={`https://tiktok.com/@${modal.data.socials.tt}`} target="_blank" className="text-zinc-500 hover:text-white transition-colors text-white">
                                <TikTokIcon size={20} />
                              </a>
                            )}
                            {modal.data.socials.yt && (
                              <a href={`https://youtube.com/@${modal.data.socials.yt}`} target="_blank" className="text-zinc-500 hover:text-white transition-colors text-white">
                                <Youtube size={20} />
                              </a>
                            )}
                          </div>
                ...
                <div className="flex items-center justify-center py-20 text-white">
                  <div className="prose prose-invert max-w-none mono text-xl leading-relaxed tracking-widest uppercase text-zinc-400 italic text-white">
                    Info coming soon...
                  </div>
                </div>
              )}
            ...
            <div className="p-8 bg-zinc-900/50 border-t border-white/5 text-center text-white">
              <p className="mono text-[8px] text-zinc-600 uppercase font-black tracking-widest text-white text-white text-white">djmerkone MUSIC // Ver. 2026.04</p>
            </div>
          </div>
        </div>
      )}
