    ...
    <div className="min-h-screen bg-[#030303] text-zinc-100 font-sans selection:bg-red-500/30 overflow-x-hidden">
      <style>
        {`
          ...
          .hub-bg { background-image: linear-gradient(to right, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.85) 40%, rgba(0,0,0,0) 100%), url("/loumerk.jpg"); background-size: cover; background-position: right center; }
      ...
      {/* Catalog (Featured + Discography Button) */}
          ...
          <div className="flex flex-col space-y-12">
            ...
            <div className="group relative flex items-center p-12 border border-white/10 bg-zinc-950 shadow-2xl transition-all text-white rounded-[2.5rem]">
              ...
              <div className="flex-grow text-white">
                <h5 className="text-3xl font-black uppercase group-hover:text-red-500 tracking-tighter text-white">
                  <DynamicShadowText text={featuredRelease.title} mousePos={mousePos} />
                </h5>
                <div className="flex items-center space-x-4 mt-3 text-white">
                  <p className="mono text-lg text-zinc-400 uppercase italic font-bold text-white text-white">{featuredRelease.artist}</p>
                ...
                <div className="flex items-center space-x-5 mt-8 text-white text-white">
                  {featuredRelease.yt && (
              ...
              <div className="flex items-center text-white text-white">
                <div 
                  ...
                  className="w-16 h-16 border border-white/10 flex items-center justify-center rounded-full group-hover:bg-red-600 group-hover:border-red-600 group-hover:text-white transition-all cursor-pointer text-white text-white"
                >
      ...
      <footer className="bg-black py-40 px-8 md:pl-40 md:pr-24 border-t border-white/5 relative overflow-hidden text-white text-white text-white">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-20 text-white text-white">
          ...
          <div className="grid grid-cols-2 gap-20 text-white text-white">
            <div className="text-white text-white">
              ...
              <ul className="space-y-4 text-xs font-black uppercase tracking-widest text-zinc-400 italic text-white text-white text-white">
                ...
                <li><a href="https://djmerkone-site0.vercel.app" target="_blank" className="hover:text-red-500 flex items-center transition-colors text-white text-white lowercase italic text-white text-white text-white">djmerkone <ExternalLink size={12} className="ml-2" /></a></li>
              </ul>
            </div>
            <div className="text-white text-white">
              ...
              <div className="flex space-x-6 text-zinc-500 text-white text-white text-white">
                <a href="https://facebook.com/djmerkone" target="_blank" className="hover:text-white transition-all text-white text-white"><Facebook size={20} /></a>
                <a href="https://instagram.com/djmerkone" target="_blank" className="hover:text-white transition-all text-white text-white"><Instagram size={20} /></a>
                <a href="https://tiktok.com/@djmerkone" target="_blank" className="hover:text-white transition-all text-white text-white"><TikTokIcon size={20} /></a>
                <a href="https://youtube.com/@djmerkone" target="_blank" className="hover:text-white transition-all text-white text-white"><Youtube size={20} /></a>
              </div>
            </div>
          </div>
        </div>
        <div className="relative z-10 pt-40 flex flex-col md:flex-row justify-between items-center text-zinc-700 mono text-[9px] uppercase tracking-widest font-black text-white text-white text-white">
          ...
          <div className="flex space-x-12 mt-8 md:mt-0 pointer-events-auto text-white text-white">
             <button onClick={() => openModal('privacy')} className="hover:text-white transition-colors cursor-pointer text-white text-white">Privacy_Desk</button>
             <button onClick={() => openModal('terms')} className="hover:text-white transition-colors cursor-pointer text-white text-white">Terms_Of_Sound</button>
          </div>
        </div>
      </footer>
      ...
      {/* Modals */}
      {modal.isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 md:p-12 transition-all duration-500 text-white text-white">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-ultra" onClick={closeModal} />
          <div className={`relative w-full ${modal.type === 'discography' ? 'max-w-7xl h-[90vh]' : 'max-w-5xl max-h-[90vh]'} bg-zinc-950 border border-white/10 rounded-[3rem] overflow-hidden flex flex-col shadow-2xl shadow-black animate-in fade-in zoom-in duration-300 text-white text-white text-white text-white text-white`}>
            <div className="p-8 border-b border-white/5 flex items-center justify-between text-white text-white text-white">
              <div className="flex items-center space-x-4 text-white text-white text-white text-white text-white">
                <div className="w-10 h-10 bg-white flex items-center justify-center text-black"><Music size={18} /></div>
                <div className="mono text-[10px] tracking-[0.4em] uppercase text-zinc-500 text-white text-white text-white text-white text-white text-white">
              ...
              <button onClick={closeModal} className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-white hover:text-black transition-all text-white text-white text-white text-white"><X size={20} /></button>
            </div>
            
            <div className="flex-grow overflow-y-auto p-0 custom-scrollbar text-white text-white text-white text-white text-white text-white text-white text-white">
              {modal.type === 'artist' ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start text-white text-white text-white text-white text-white p-10 text-white">
                  <div className="relative aspect-[4/5] bg-zinc-900 overflow-hidden rounded-[2rem] text-white text-white text-white">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-black to-transparent text-white text-white text-white text-white text-white text-white text-white text-white">
                      <h3 className={`text-6xl font-black uppercase tracking-tighter italic text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white ${modal.data.name === 'djmerkone' ? 'lowercase' : ''}`}>{modal.data.name}</h3>
                      <div className="flex flex-wrap gap-2 mt-4 opacity-70 text-white text-white text-white text-white text-white text-white text-white">
                  ...
                  <div className="space-y-12 text-white text-white text-white text-white text-white text-white text-white">
                    <div className="prose prose-invert max-w-none text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
                      {modal.data.isMemorial && (
                        <div className="mb-10 p-6 bg-red-900/10 border-l-4 border-red-900 rounded-r-2xl text-white text-white text-white text-white text-white text-white">
                          <p className="mono text-[10px] text-red-500 font-bold uppercase tracking-widest mb-2 flex items-center gap-2 text-white text-white text-white"><Heart size={12} fill="currentColor" /> Legacy Memorandum</p>
                          <p className="text-zinc-300 font-black tracking-tighter text-xl italic text-white text-white text-white text-white text-white text-white text-white">{modal.data.note}</p>
                        </div>
                      )}
                      <p className="mono text-[11px] leading-relaxed tracking-widest uppercase text-zinc-400 italic whitespace-pre-line text-white text-white text-white text-white text-white text-white text-white text-white">{modal.data.bio}</p>
                    </div>
                    <div className="pt-12 border-t border-white/5 text-white text-white text-white text-white text-white text-white text-white">
                      <h4 className="mono text-[10px] tracking-[0.4em] uppercase text-red-500 mb-8 font-bold italic text-white text-white text-white text-white text-white text-white text-white">Official_Connectivity</h4>
                      <div className="grid grid-cols-1 gap-6 text-white text-white text-white text-white text-white text-white text-white">
                        {modal.data.link && (
                          <a href={modal.data.link} target="_blank" className="flex items-center space-x-6 group text-white text-white text-white text-white text-white text-white text-white">
                            <div className="w-10 h-10 border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-all text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white"><Globe size={16} className="text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white" /></div>
                            <span className="text-sm font-black uppercase tracking-widest italic underline decoration-zinc-800 group-hover:decoration-white transition-all underline-offset-8 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">{modal.data.isMemorial ? 'Memorial Obituary' : 'Official Website'}</span>
                          </a>
                        )}
                        {modal.data.socials && (
                          <div className="flex flex-wrap gap-6 pt-4 text-white text-white text-white text-white text-white text-white text-white">
                            {Object.entries(modal.data.socials).map(([key, val]) => (
                              <a key={key} href={`https://${key}.com/${val}`} target="_blank" className="text-zinc-500 hover:text-white transition-colors text-white text-white">
                ...
                <div className="p-10 lg:p-20 text-white">
                  <div className="relative mb-12">
                    ...
                    <input 
                      ...
                      className="w-full bg-black/40 border border-white/10 p-6 pl-16 rounded-full mono text-xs uppercase tracking-widest text-zinc-400 focus:outline-none focus:border-red-500/50 transition-all"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                    {filteredDiscography.map((group, idx) => (
                      <div key={idx} className="space-y-6">
                        <h4 className="text-lg font-black text-red-600 uppercase tracking-[0.2em] italic border-b border-red-900/20 pb-2">{group.artist}</h4>
                        <ul className="space-y-3">
                          {group.tracks.map((track, tIdx) => (
                            <li key={tIdx} className="mono text-[10px] text-zinc-500 uppercase tracking-widest leading-relaxed hover:text-white transition-colors flex items-start">
                              <span className="mr-3 text-zinc-800">0{tIdx + 1}</span>
                              {track}
                            </li>
                          ))}
                        </ul>
              ...
              ) : <div className="flex items-center justify-center py-20 text-white text-white text-white text-white text-white text-white text-white text-white"><div className="prose prose-invert max-w-none mono text-xl leading-relaxed tracking-widest uppercase text-zinc-400 italic text-white text-white text-white text-white text-white text-white text-white text-white text-white">Info coming soon...</div></div>)}
            </div>
            
            <div className="p-8 bg-zinc-900/50 border-t border-white/5 text-center text-white text-white text-white text-white text-white text-white text-white"><p className="mono text-[8px] text-zinc-600 uppercase font-black tracking-widest text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">djmerkone MUSIC // Ver. 2026.04</p></div>
          </div>
        </div>
      )}
    </div>
